...
export default function JobSearchPage() {
  ...
  const searchJobs = async (keyword, city, page = 1) => {
    try {
      ...
      if (!appId || !appKey) {
        console.error('Adzuna API credentials not found in environment variables');
      ...
      if (city && (city.toLowerCase().includes('saskatoon') || 
    ...
    <div className="resume-page-wrapper">
      ...
      {showApplyModal && (
        <div style={{
          ...
          <div style={{ background: 'white', borderRadius: 12, padding: '1.2rem 1.2rem 1rem 1.2rem', minWidth: 280, maxWidth: 340, boxShadow: '0 4px 24px rgba(0,0,0,0.18)', color: '#222', position: 'relative' }}>
            <button onClick={() => setShowApplyModal(false)} style={{ position: 'absolute', top: 12, right: 16, background: 'none', border: 'none', fontSize: 22, color: '#888', cursor: 'pointer' }}>&times;</button>
            <h2 style={{ marginBottom: 12, fontWeight: 700, fontSize: '1.2rem', color: '#003049' }}>Apply for {selectedJob?.title}</h2>
            <form onSubmit={handleApply}>
              <select
                value={selectedResume}
                onChange={e => setSelectedResume(e.target.value)}
                required
                style={{ width: '100%', padding: '0.5rem', borderRadius: 8, border: '1px solid #bbb', marginBottom: 16 }}
              >
                <option value="" disabled>Select your resume</option>
                {resumes.map((resume, idx) => (
                  <option key={idx} value={resume.title}>{resume.title} ({resume.lastEdited})</option>
                ))}
              </select>
              <button type="submit" className="job-action" style={{ width: '100%', background: '#0077b6', color: 'white', marginTop: 8 }}>
                Apply
              </button>
              {applySuccess && <div style={{ color: '#28a745', fontWeight: 600, marginTop: 12 }}>Application submitted!</div>}
              {applyError && <div style={{ color: '#d90429', fontWeight: 600, marginTop: 12 }}>{applyError}</div>}
            </form>
          </div>
        </div>
      )}
